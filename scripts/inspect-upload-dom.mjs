...
await sleep(3500);
console.log('上传页路径:', await evalJs('location.pathname', sessionId));
console.log('\n=== 输入框 ===');
console.log(await evalJs(`JSON.stringify([...document.querySelectorAll('input,textarea')].map((i,idx)=>({
  idx, tag:i.tagName, type:i.type, placeholder:i.placeholder, ariaLabel:i.getAttribute('aria-label'), visible: i.offsetParent!==null
})),null,1)`, sessionId));
console.log('\n=== 按钮 ===');
console.log(await evalJs(`JSON.stringify([...document.querySelectorAll('button')].filter(b=>b.offsetParent).map((b,idx)=>({
  idx, text:b.textContent.trim().slice(0,20), type:b.type, disabled:b.disabled
})),null,1)`, sessionId));
