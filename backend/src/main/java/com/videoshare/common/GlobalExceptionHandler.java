...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiResponse<Void>> unknown(Exception ex) { return ResponseEntity.internalServerError().body(ApiResponse.error(ErrorCode.INTERNAL, "服务暂时不可用")); }
    private int status(ErrorCode code) { return switch (code) { case UNAUTHORIZED, TOKEN_INVALID -> 401; case FORBIDDEN -> 403; case NOT_FOUND -> 404; case RATE_LIMITED -> 429; case INTERNAL -> 500; default -> 400; }; }
}
